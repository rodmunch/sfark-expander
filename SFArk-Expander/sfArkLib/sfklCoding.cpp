...
#undef FIXENDIAN
        ...
        *(uint32_t *)(HdrBuf+HEADER_HDRCHECK_POS) = 0;			// Zero-out the HeaderChecksum position in the buffer
        CalcHdrCheck = adler32(0, HdrBuf, (unsigned int)HeaderLen);		// and recalculate the header checksum
        HdrCheckDone = true;
...
int DecompressTurbo(BLOCK_DATA *Blk, USHORT NumWords)
{
    ...
    long UnCrunchResult = UnCrunchWin(Blk->SrcBuf, NumWords, 8*OPTWINSIZE);
    if (UnCrunchResult < 0)
    {
        sprintf(MsgTxt, "ERROR - UnCrunchWin returned: %ld %s", UnCrunchResult, CorruptedMsg);
        msg(MsgTxt, MSG_PopUp);
...
#define	AWBYTES	(sizeof(AWORD))
    ...
    switch (Blk->FileSection)
    {
        case AUDIO:
        {
            ...
            {
                n = (uint32_t)(Blk->FileHeader.PostAudioStart - Blk->TotBytesWritten);				// Get exact length in bytes
...
#undef WFIX
        ...
        case PRE_AUDIO: case POST_AUDIO: case NON_AUDIO:
        {
            ...
            m = (uint32_t)UnMemcomp(zSrcBuf, n, zDstBuf, ZBUF_SIZE);											//printf("PRE/POST AUDIO block, compressed %ld bytes, uncompressed %ld bytes\n", n, m);
...
bool	ExtractTextFile(BLOCK_DATA *Blk, ULONG FileType)
{
    ...
    m = UnMemcomp(zSrcBuf, (int)n, zDstBuf, ZBUF_SIZE);										// Uncompress
    Blk->FileCheck = adler32(Blk->FileCheck, zDstBuf, (int)m);	   					// Accumulate checksum
