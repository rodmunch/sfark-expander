...
/*#if 1
    __int64_t i;
    size_t lenInFileName = strlen(InFileName);
...
//#endif
    ...
    OutputFileHandle = fopen(OutFileName, "wb"); //CREATEFILE(OutFileName);
...
long ReadInputFile(BYTE *Buf, long BytesToRead)
{
  long BytesRead;
...
long WriteOutputFile(const BYTE *Buf, long BytesToWrite)
{
  size_t	BytesWritten;
...
bool SetInputFilePosition(long NewPos)
{
...
bool SetOutputFilePosition(long NewPos)
{
